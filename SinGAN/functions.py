...
def calc_gradient_penalty(netD, real_data, fake_data, LAMBDA): # Notice that gradient penalty only works in D's loss function
...
def adjust_scales2image(real_,opt):
    opt.num_scales = int((math.log(math.pow(opt.min_size / (real_.shape[2]), 1), opt.scale_factor_init))) + 1 ## 14
    scale2stop = int(math.log(min([opt.max_size, max([real_.shape[2], real_.shape[3]])]) / max([real_.shape[2], real_.shape[3]]),opt.scale_factor_init)) ## 5
    opt.stop_scale = opt.num_scales - scale2stop ## 9

    opt.scale1 = min(opt.max_size / max([real_.shape[2], real_.shape[3]]),1)  # min(256/max([real_.shape[0],real_.shape[1]]),1) , 0.213
    real = imresize(real_, opt.scale1, opt) # resize the image based on the scale e.g. real.shape = [1, 3, 256, 256]

    opt.scale_factor = math.pow(opt.min_size / (real.shape[2]), 1 / (opt.stop_scale))  # 0.772
    return real
